...
for (i in seq_along(seurat_objects)) {
  obs_names = colnames(seurat_objects[[i]])
  tissue = list()
  ...
  seurat_objects[[i]]@meta.data[['tissue']] <- tissue
  ...
  seurat_objects[[i]] <- subset(seurat_objects[[i]], tissue != 'blood')
}
...
integrated_data = readRDS('integrated_data.rds')
barcode_data <- read.csv("GSE161277_gd.csv")
...
new_seurat_object <- CreateSeuratObject(wanted_mat, project = 'GSE161277' )
new_seurat_object = AddMetaData(new_seurat_object, metadata = barcode_data )
saveRDS(new_seurat_object, 'GSE161277_gd.rds')

integrated_data = readRDS('integrated_data.rds')
barcode_data <- read.csv("GSE161277_T17.csv")
merged = JoinLayers(integrated_data@assays$RNA)
wanted_mat <- merged@layers$counts[, colnames(merged) %in% barcode_data$X]
keep_cell_barcodes = colnames(merged)[colnames(merged) %in% barcode_data$X]
colnames(wanted_mat) = keep_cell_barcodes
...
new_seurat_object <- CreateSeuratObject(wanted_mat, project = 'GSE161277' )
new_seurat_object = AddMetaData(new_seurat_object, metadata = barcode_data )
saveRDS(new_seurat_object, 'GSE161277_T17.rds')
