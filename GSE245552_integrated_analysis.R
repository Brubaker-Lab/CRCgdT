...
saveRDS(seurat_objects, 'GSE245552/COLON/seurat_objects.rds')
seurat_objects = readRDS('GSE245552/COLON/seurat_objects.rds')
saveRDS(anchors, 'GSE245552/COLON/anchors.rds')
anchors = readRDS('GSE245552/COLON/anchors.rds')
saveRDS(integrated_data, 'GSE245552/COLON/integrated_data.rds')
integrated_data = readRDS('GSE245552/COLON/integrated_data.rds')
...
integrated_data = readRDS('GSE245552/integrated_data.rds')
barcode_data <- read.csv("GSE245552_gd.csv")
...
new_seurat_object <- CreateSeuratObject(wanted_mat, project = 'GSE245552' )
new_seurat_object = AddMetaData(new_seurat_object, metadata = barcode_data )
saveRDS(new_seurat_object, 'GSE245552_gd.rds')

integrated_data = readRDS('GSE245552/integrated_data.rds')
barcode_data <- read.csv("GSE245552_T17.csv")
merged = JoinLayers(integrated_data@assays$RNA)
wanted_mat <- merged@layers$counts[, colnames(merged) %in% barcode_data$X]
keep_cell_barcodes = colnames(merged)[colnames(merged) %in% barcode_data$X]
colnames(wanted_mat) = keep_cell_barcodes
...
new_seurat_object <- CreateSeuratObject(wanted_mat, project = 'GSE245552' )
new_seurat_object = AddMetaData(new_seurat_object, metadata = barcode_data )
saveRDS(new_seurat_object, 'GSE245552_T17.rds')


srat = readRDS('GSE188711_T17.rds')
srat@meta.data$orig.ident
