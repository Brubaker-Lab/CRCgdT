...
main_dir <- ""
# List all folders in the main directory starting with "GSM"
folders <- list.dirs(path = main_dir, full.names = TRUE, recursive = FALSE)
gsm_folders <- folders[grep("^GSM", basename(folders))]
cond_names = list()
for (folder in gsm_folders) {
...
integrated_data = readRDS('integrated_data.rds')
barcode_data <- read.csv("GSE188711_LeftnRight_gd.csv")
...
new_seurat_object <- CreateSeuratObject(wanted_mat, project = 'GSE188711_LeftnRight')
new_seurat_object = AddMetaData(new_seurat_object, metadata = barcode_data )
saveRDS(new_seurat_object, 'GSE188711_LeftnRight_gd.rds')
...
colnames(wanted_mat) = keep_cell_barcodes
rownames(wanted_mat) = rownames(merged)
new_seurat_object <- CreateSeuratObject(wanted_mat, project = 'GSE188711')
new_seurat_object = AddMetaData(new_seurat_object, metadata = barcode_data )
saveRDS(new_seurat_object, 'GSE188711_T17.rds')
